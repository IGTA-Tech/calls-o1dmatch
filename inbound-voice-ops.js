/**
 * Inbound Voice Handler - OPERATIONS MODE
 * Updated March 2026: Phone is for BD only, existing clients → email
 * 
 * Key Changes:
 * - All support requests directed to email
 * - 1-3 business day response time expectation
 * - Phone reserved for new business development
...
// Load prompts from files (operations mode)
function loadPrompt(brand) {
  const promptFile = path.join(__dirname, 'prompts', `${brand}-ops-mode.txt`);
  const fallbackFile = path.join(__dirname, 'prompts', `${brand}-enhanced.txt`);
  ...
  try {
    ...
    console.error(`Error loading prompt for ${brand}:`, err);
  }
  return null;
}

// Brand configurations - each number has its own personality
// Uses operations mode prompts
const BRAND_CONFIG = {
  ...
  '+19803032854': {
    ...
    voice: 'nat',
    email: 'gabriella@sherrodsportsvisas.com',
    greeting: "Hey there! This is Sevyn. I work with Sherrod Seward and his team. How can I help you?"
  },
 ...
 * Uses operations mode prompts that direct to email
 */
async function createInboundAICall(callerNumber, calledNumber, options = {}) {
  ...
  // Load the operations mode prompt
  let prompt = loadPrompt(brand.brandKey);
  if (!prompt) {
    // Fallback prompt if file not found
    prompt = `You are ${brand.name}'s AI receptionist. 
    
IMPORTANT: We're in operations mode. Phone is for new business only.
...
- Take messages for new leads, explain email follow-up`;
  ...
  if (vip) {
    prompt = `IMPORTANT: This caller is ${vip.name} (${vip.relation}). Greet them by name and be extra helpful. For ${vip.name}, you can be more flexible - but still encourage email for tracking purposes.\n\n${prompt}`;
  }
  ...
  const callConfig = {
    ...
    analysis_schema: {
      ...
      follow_up_needed: "boolean - whether someone needs to call them back",
      told_to_email: "boolean - whether caller was directed to email",
  ...
  console.log(`   📧 Will direct to: ${brand.email}`);
 ...
 * Operations Mode: Brief greeting, then connect to AI
 */
...
function handleInboundTransfer(req, res) {
  ...
  console.log(`\n📞 INBOUND CALL (OPS MODE - Transfer)`);
  console.log(`   From: ${callerNumber}${vip ? ` (VIP: ${vip.name})` : ''}`);
  console.log(`   To: ${calledNumber} (${brand.brand})`);
  ...
  const blandInboundNumber = '+19803502728';
  twiml.dial({ 
    timeout: 30,
